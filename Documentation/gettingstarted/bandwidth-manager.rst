...
control for Pods as outlined further below.
...
latencies for Internet traffic, for example, it has been shown that BBR's throughput
can reach as much as 2,700x higher than today's best loss-based congestion control;
...
to get BBR for Pods working. BBR also needs eBPF Host-Routing in order to being able
to retain the network packet's socket association all the way until the packet hits
...
for other connections using Cubic. We recommend to use BBR in particular for clusters
where Pods are exposed as Services which serve external clients connecting from the
Internet.
