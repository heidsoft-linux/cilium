...
shown that BBR's throughput can reach as much as 2,700x higher than today's best
loss-based congestion control; queueing delays can be 25x lower.
...
BBR also needs eBPF Host-Routing in order to being able to retain the network packet's
socket association all the way until the packet hits the FQ queueing discipline on the
